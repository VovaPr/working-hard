...
def run_pipeline(api: PipelineApi):
    ...
    _phase(api.version, "core.images", f"Done in {images_elapsed:.2f}s")

    _phase(api.version, "core.gif", "Start GIF/animated WEBP processing")
    gifs_started_at = time.time()
    api.process_gifs(gif_paths, animated_webp_paths)
    gifs_elapsed = time.time() - gifs_started_at
    _phase(api.version, "core.gif", f"Done in {gifs_elapsed:.2f}s")

    print(
        f"{api.version} | [core.scan] Completed: scan_media={api.run_metrics['scan_sec']:.2f} sec "
    ...
    try:
        ...
        if stats_size_mb >= api.stats_soft_limit_mb:
            print(
                f"{api.version} | [core.stats] Warning: {os.path.basename(api.stats_file)} is {stats_size_mb:.2f} MB "
    ...
    try:
        _phase(api.version, "core.stats", "Run stats compressor")
    ...
    print(
        f"{api.version} | [core.summary] Scan time: {api.run_metrics['scan_sec']:.2f} sec. "
        f"Total number of files in folder: {total_files_in_dir}"
    )
    print("All images converted/compressed and oversized GIFs/Webps processed.")
