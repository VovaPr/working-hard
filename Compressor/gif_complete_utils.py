...
def _advance_fast_only_scale(*, state, fast_size, target_mid, gif_cfg, version):
    ...
    if fast_size < gif_cfg.targets.target_min_mb:
        state.low_scale = max(state.low_scale, state.scale)
    elif fast_size > gif_cfg.targets.target_max_mb:
    ...
    max_step_ratio = max(0.30, gif_cfg.runtime.max_scale_step_ratio * 2.0) / state.scale if state.scale > 0 else 0.30
