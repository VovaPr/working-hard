from compressor_gif_runtime import build_skip_decision, predict_medcut_size
...
def _handle_medcut_disabled_path(
    *,
    iteration,
    state,
    fast_size,
...
def _run_fast_trial_stage(
    *,
    ...
    if _try_fast_accept(
        ...
        return {"status": "done", "frames_raw": frames_raw, "durations": durations, "total_frames": total_frames}

    if state.medcut_disabled:
        terminal_status = _handle_medcut_disabled_path(
            iteration=iteration,
            state=state,
            fast_size=fast_size,
            fast_bytes=fast_bytes,
        ...
        return {"status": terminal_status, "frames_raw": frames_raw, "durations": durations, "total_frames": total_frames}

    return {"status": "ready", "resized_frames": resized_frames, "fast_size": fast_size, "fast_bytes": fast_bytes}


def _probe_and_build_skip_decision(
    *,
    iteration,
    source,
    source_is_neighbor,
...
def _terminal_prepare_result(*, status, frames_raw, durations, total_frames):
    return {
        "status": status,
        "frames_raw": frames_raw,
        "durations": durations,
...
def _ready_prepare_result(*, resized_frames, fast_size, fast_bytes, predicted_medcut, frames_raw, durations, total_frames):
    return {
        "status": "ready",
        "resized_frames": resized_frames,
        "fast_size": fast_size,
